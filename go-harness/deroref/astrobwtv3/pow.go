...
func AstroBWTv3(input []byte) (outputhash [32]byte) {
	...
	HxShaKey = sha_key // harness: prologue step 1

	salsa.XORKeyStream(step_3[:], step_3[:], &counter, &sha_key)
	HxPostSalsa = step_3 // harness: prologue step 2
	...
	HxPostRc4 = step_3 // harness: prologue step 3
	lhash := fnv1a.HashBytes64(step_3[:])
	HxLhashInitial = lhash // harness: prologue step 4 (initial lhash)
	...
	// harness: op-loop intermediates + the stream fingerprint
	HxTries = tries
	HxDataLen = data_len
	HxLhashFinal = lhash
	HxPrevLhash = prev_lhash
	HxStep3Final = step_3
	HxDataHash = sha256.Sum256(scratch.data[:data_len])
